...
function base62ToBytes(str: string): Uint8Array {
  ...
  for (let i = SOUL_ID_BYTE_LEN - 1; i >= 0; i--) {
...
function unpackBits(buf: Uint8Array, schema: number[]): number[] {
  ...
  for (const bits of schema) {
    ...
    for (let b = bits - 1; b >= 0; b--) {
      if (
        pos < buf.length * 8 &&
        (buf[Math.floor(pos / 8)] >> (7 - (pos % 8))) & 1
      )
        v |= 1 << b;
      pos++;
    }
...
export function encodeSoulId(
  ...
  const age = Math.min(
    Math.max(parseInt(demographics.age || "22") - 18, 0),
    82,
  );
  const fields = [
    ...
    {
      value: Math.max(0, INTERESTED_LIST.indexOf(demographics.interestedIn)),
      bits: 2,
    },
    ...
    ...questions.map((q) => {
      ...
      return {
        value: Math.min(Math.max((ans?.value ?? 3) - 1, 0), 4),
        bits: 3,
      };
    }),
  ];
...
export function decodeSoulId(soulId: string): {
  ...
  try {
    ...
    return {
      ...
      answerValues: rawAnswers
        .slice(0, 60)
        .map((v) => Math.min(Math.max(v + 1, 1), 5)),
...
export const generateQuizQuestions = async (
  ...
  dimensions.forEach((dim) => {
    const shuffled = seededShuffle(
      MASTER_QUESTION_BANK.filter((q) => q.dimension === dim),
      actualSeed,
    );
    shuffled.slice(0, 12).forEach((q) =>
...
const LOCAL_FALLBACK: Record<Language, AIResult> = {
  en: {
    ...
    summary:
      "Your soul profile is crystallizing — connect to the network for a full deep analysis.",
    idealPartner: {
      description: "Someone who aligns with your core truth.",
      traits: ["Empathy", "Respect", "Growth"],
      dealBreakers: ["Dishonesty", "Stagnation"],
    },
  ...
  ja: {
    mbtiType: "調和のとれた旅人",
    summary:
      "あなたの魂の映像が形になりつつあります。ネットワークに接続して完全な分析を取得してください。",
    idealPartner: {
      description: "あなたの核心的な真実と一致する人。",
      traits: ["共感", "尊重", "成長"],
      dealBreakers: ["不誠実", "停滞"],
    },
    compatibilityAdvice:
      "オンラインになったらリロードして個別のガイダンスを取得してください。",
  },
};
...
const callSoulAI = async (
  ...
  // Filter and sort by importance (distance from neutral 3)
  const keyQuestions = [...questions].sort((a, b) => {
    const valA = answers.find((ans) => ans.questionId === a.id)?.value ?? 3;
    const valB = answers.find((ans) => ans.questionId === b.id)?.value ?? 3;
    return Math.abs(valB - 3) - Math.abs(valA - 3);
  }).slice(0, 20); // Only send top 20 most defining questions

  const transcript = keyQuestions
    .map((q) => {
      const ans = answers.find((a) => a.questionId === q.id);
  ...
  const schema: Schema = {
    ...
    properties: {
      ...
      mbtiType: {
        type: Type.STRING,
        description:
          "A rich, evocative archetype title (e.g. 理性实用主义者, 进取心极强的创业者, 浪漫理想家)",
      },
      idealPartner: {
        type: Type.OBJECT,
        properties: {
  ...
  try {
    ...
    const reqContents = `You are a world-class relationship psychologist and insightful observer of human souls. Analyze the most defining answers from this user's "Soul Journey" profile to understand their core values.
Input: Age/Gender/Seeking: ${demographics.age}, ${demographics.gender}, ${demographics.interestedIn}
Key Answers indicating strong personality traits (1=Strongly Disagree, 5=Strongly Agree):
${transcript}
...
Output Language: ${langName}. ALL JSON string values MUST be in ${langName}. Use "你" (You) for all descriptions.`;
    ...
    if (tokenCountRes.totalTokens && tokenCountRes.totalTokens > 150000) {
      console.warn(`CallSoulAI: Token count ${tokenCountRes.totalTokens} exceeds 150,000 limit. Triggering fallback.`);
      return null;
    }
    ...
    try {
      ...
      console.warn(
        "CallSoulAI: Error getting text from response object",
        parseErr,
      );
      text = response.candidates?.[0]?.content?.parts?.[0]?.text || "";
    }
    ...
    if (text) {
      console.log(
        "CallSoulAI: Successfully extracted text",
...
export const getProfileBySoulId = async (
  soulId: string,
  ...
  const answers: QuizAnswer[] = decoded.answerValues.map((v, i) => ({
    ...
    value: v as import("../types").LikertValue,
  }));
  ...
  const aiResult =
    (await callSoulAI(
      ...
      answers,
      decoded.language,
    )) ?? LOCAL_FALLBACK[decoded.language];
  const scores = calculateScores(questions, answers, decoded.language);
  const profile: MatchProfile = {
    soulId,
    summary: aiResult.summary,
    mbtiType: aiResult.mbtiType,
    scores,
    idealPartner: aiResult.idealPartner,
    compatibilityAdvice: aiResult.compatibilityAdvice,
    timestamp: Date.now(),
...
export const analyzeProfile = async (
  ...
  const aiResult =
    (await callSoulAI(demographics, questions, answers, language)) ??
    LOCAL_FALLBACK[language];
...
export const activateQuizAttempt = async (
  attempt: import("../types").QuizAttempt,
) => {
  const user = await getCurrentUser();
